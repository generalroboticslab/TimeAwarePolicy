...
from core.common.time import format_timestamp
...
def _blueprint_auto_view_option():
    """Return the equivalent auto-view switch for Rerun 0.20 and 0.21."""
    parameters = inspect.signature(rrb.Blueprint).parameters
    option = "auto_views" if "auto_views" in parameters else "auto_space_views"
    return {option: False}
    ...
    def log_timeaware_step(
        ...
        *,
        ...
        if observed_tte is not None:
            ...
            real_tte = (
                format_timestamp(real_tte) if real_tte is not None else "N/A"
            )
            rr.log(
                "time_text",
                rr.TextDocument(
