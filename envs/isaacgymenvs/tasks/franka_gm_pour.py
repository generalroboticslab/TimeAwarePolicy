...
class FrankaGmPour(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # Create cups
        n_slices = 6
        # Changing cup size also requires adjusting the grasp pose.
        thickness = 0.005
        ...
        # The deterministic cup geometry is bundled with the release. Loading
        # it directly keeps environment construction read-only and avoids
        # rewriting tracked assets on every process start.
        cupA_asset_fpath = cupB_asset_fpath = "urdf/procedural/cup.urdf"
        if self.cfg.get("use_container", False):
            ...
            cupB_asset_fpath = "urdf/procedural/cupB.urdf"
        for relative_asset in {
            cupA_asset_fpath,
            cupB_asset_fpath,
            "urdf/procedural/real_cup.stl",
        }:
            asset_path = os.path.join(asset_root, relative_asset)
            if not os.path.isfile(asset_path):
                raise FileNotFoundError(
                    f"required checked-in GM Pour asset is missing: {asset_path}"
                )
    ...
    def reset_idx(self, env_ids):
        ...
        if len(new_cfg_env_ids) > 0:
            ...
            self._apply_knn_reference_targets(
                new_cfg_env_ids,
                {
                    "cupA_state": self._init_cupA_state[new_cfg_env_ids],
                    "cupB_state": self._init_cupB_state[new_cfg_env_ids],
