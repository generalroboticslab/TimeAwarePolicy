...
from core.common.tf_numpy import tf_inverse as tf_inverse_np, tf_combine as tf_combine_np
from envs.isaacgymenvs.tasks.utils.object_utils import create_hollow_cylinder
from copy import deepcopy
from envs.isaacgymenvs.task_semantics import cube_gripper_clearance
...
class FrankaCubeStack(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        restitution = 1. if self.cfg.get("add_restitution", False) else 0.1
    ...
    def compute_task_reward(self):
        ...
        gripper_away_from_cubeA = d > cube_gripper_clearance(self.cfg)
        success_conditions = cubeA_align_cubeB & cubeA_on_cubeB & gripper_away_from_cubeA
        if self.cfg.get("apply_disturbances", False) or self.cfg.get("add_restitution", False):
            success_conditions = success_conditions & (cubeA_linvel_norm < 0.05)
        ...
        rewards = reward_settings["r_dist_scale"] * dist_reward + reward_settings["r_lift_scale"] * lift_reward + reward_settings["r_align_scale"] * align_reward
    ...
    def reset_idx(self, env_ids):
        ...
        if len(new_cfg_env_ids) > 0:
            ...
            self._apply_knn_reference_targets(
                new_cfg_env_ids,
                {
                    "cubeA_state": self._init_cubeA_state[new_cfg_env_ids],
                    "cubeB_state": self._init_cubeB_state[new_cfg_env_ids],
                    "franka_state": self.fk_init_dof[new_cfg_env_ids],
                },
            )
