...
def create_link(name, subelements=None, mass=0.01, origin=(0, 0, 0)):
    """
    ...
    ET.SubElement(inertial, "origin", xyz=convert_to_string(origin), rpy="0 0 0")
    ET.SubElement(inertial, "mass", value=convert_to_string(mass))
    # We need to compute the inertial based on the object's geometry later!
...
def create_hollow_cylinder(name, size, thickness, mass, n_slices=8, shape="round", use_lid=False,
    ...
    for create_body in (create_collision_body, create_visual_body):
        ...
        if (create_body != create_visual_body) or (not transparent_walls):
            ...
            if shape == "round":
                ...
                wall_length = 2 * radius + thickness
            for i in range(n_slices):
                angle = np.pi * 2 * (i / n_slices)
                pos_x = 0 if np.isclose(radius * np.cos(angle), 0, atol=1e-6) else radius * np.cos(angle)
                pos_y = 0 if np.isclose(radius * np.sin(angle), 0, atol=1e-6) else radius * np.sin(angle)
                wall_bodies.append(
                    create_body(
                        pos=(pos_x, pos_y, 0),
                        rpy=(0, 0, angle),
    ...
    # Create links from base bodies and wall bodies
    base_link = create_link(name=f"{name}_base", subelements=base_bodies, mass=base_mass, origin=(0, 0, thickness / 2))
    wall_link = create_link(name=f"{name}_wall", subelements=wall_bodies, mass=wall_mass, origin=(0, 0, thickness / 2))
    # Create joint linking bodies together
    ...
    if generate_urdf:
        relative_fpath = generate_urdf_from_xmltree(
            elements=elements,
            name=name,
            dirpath=os.path.join(asset_root_path, "urdf/procedural"),
            unique_urdf=unique_urdf_name,
        )
...
def urdf_single_mesh_cylinder(
    ...
    mesh_relative_path=None,
    use_same_collision_mesh=True,
    collision_mesh_relative_path=None,
    asset_root_path=ASSETS_ROOT,
):
    """
    Create a single-link URDF referencing a pre-generated hollow-cylinder mesh.
    Link frame is at bottom-center of the cylinder (z=0 at bottom face).

    Args:
        name: URDF robot name and link name base.
        mesh_relative_path: Path to visual mesh (e.g., "urdf/procedural/cylinder/hollow_cylinder.stl").
        mass: Total mass [kg].
        radius: Cylinder outer radius [m] (for inertia approximation).
        height: Cylinder height [m] (for inertia approximation).
        mesh_scale: (sx, sy, sz) scale applied to visual and collision mesh.
        rgba: (r, g, b, a) color for visual.
        use_same_collision_mesh: If True, collision uses the same mesh as visual.
        collision_mesh_relative_path: Optional separate collision mesh path.

    Returns:
        URDF string.
    """

    # Approximate spatial inertia for a solid cylinder of mass m, radius r, height h
    # Ixx = Iyy = (1/12) m (3 r^2 + h^2), Izz = (1/2) m r^2, with origin at COM at center.
    # Our link frame is at bottom-center, so we shift inertia from COM (center) up by h/2 via parallel-axis.
    # Shift along z: d=(0,0,h/2). I_shift = I + m * (||d||^2 * I3 - d d^T)
    # => Adds m*(h/2)^2 to Ixx and Iyy; Izz unchanged.

    r = float(radius)
    h = float(height)
    m = float(mass)

    Ixx_com = (1.0 / 12.0) * m * (3.0 * r * r + h * h)
    Iyy_com = Ixx_com
    Izz_com = 0.5 * m * r * r

    dz = h / 2.0
    Ixx = Ixx_com + m * (dz * dz)   # shift
    Iyy = Iyy_com + m * (dz * dz)
    Izz = Izz_com                   # unchanged

    # Visual and collision mesh paths
    mesh_relative_path = mesh_relative_path or os.path.join(asset_root_path, "urdf/procedural/simple_cup.stl")
    create_hollow_cylinder_mesh(radius=radius, height=height, thickness=thickness, n_sides=n_sides, save_path=mesh_relative_path)
    vis_mesh = mesh_relative_path
    col_mesh = vis_mesh if use_same_collision_mesh or not collision_mesh_relative_path else collision_mesh_relative_path

    sx, sy, sz = mesh_scale
    r_, g_, b_, a_ = rgba

    # URDF with a single link and no joints. The origin of the link is at the bottom center.
    # We set the inertial origin at the link frame as well (bottom center).
...
<robot name="{name}">
  <link name="{name}_link">
    ...
    <visual>
      <origin xyz="0 0 0" rpy="0 0 0"/>
      <geometry>
        <mesh filename="{vis_mesh}" scale="{sx} {sy} {sz}"/>
      </geometry>
      <material name="{name}_mat">
        <color rgba="{r_} {g_} {b_} {a_}"/>
      </material>
    </visual>
    ...
    <collision>
      <origin xyz="0 0 0" rpy="0 0 0"/>
      <geometry>
        <mesh filename="{col_mesh}" scale="{sx} {sy} {sz}"/>
      </geometry>
    </collision>
  </link>
</robot>
"""
    ...
    # save urdf to file
    urdf_dir = os.path.dirname(mesh_relative_path)
    urdf_fpath = os.path.join(urdf_dir, f"{name}.urdf")
    with open(urdf_fpath, "w") as f:
        f.write(urdf)

    return urdf_fpath


def create_hollow_cylinder_mesh(radius, height, thickness, n_sides=32, save_path=None):
    if thickness <= 0 or thickness >= radius:
        raise ValueError("thickness must be > 0 and < radius")

    # Create outer and inner rings as polygons
    angles = np.linspace(0, 2 * np.pi, n_sides, endpoint=False)
    inner2outer = thickness / np.cos(np.pi / n_sides)
    outer = np.column_stack([radius * np.cos(angles), 
                             radius * np.sin(angles)])
    inner = np.column_stack([(radius - inner2outer) * np.cos(angles),
                             (radius - inner2outer) * np.sin(angles)])

    wall_poly = Polygon(shell=outer, holes=[inner])
    wall_mesh = trimesh.creation.extrude_polygon(wall_poly, height)

    # Create the base to close the bottom
    base_poly = Polygon(outer)
    base_mesh = trimesh.creation.extrude_polygon(base_poly, thickness)
    
    mesh = trimesh.util.concatenate([wall_mesh, base_mesh])
    mesh.fix_normals()
...
if __name__ == "__main__":
    # Create a round cup
    round_cup = create_hollow_cylinder_mesh(
        name="round_cup",
        radius=0.05,  # 5cm radius
        height=0.1,   # 10cm height
        thickness=0.005,  # 5mm thickness
        mass=0.1,     # 100g
        n_sides=32,   # Round shape
    )
    
    # Create a square cup
    square_cup = create_hollow_cylinder_mesh(
        name="square_cup",
        radius=0.05,
        height=0.1,
        thickness=0.005,
        mass=0.1,
        n_sides=4,    # Square shape
    )
    
    # Create a hexagonal cup with lid
    hex_cup = create_hollow_cylinder_mesh(
        name="hex_cup_with_lid",
        radius=0.05,
        height=0.1,
        thickness=0.005,
        mass=0.12,
        n_sides=6,    # Hexagonal shape
    )

    print(hex_cup)
    
    # You can also get mesh properties
    print(f"Round cup volume: {round_cup.volume:.6f} m³")
    print(f"Round cup center of mass: {round_cup.center_mass}")
    print(f"Round cup inertia:\n{round_cup.moment_inertia}")