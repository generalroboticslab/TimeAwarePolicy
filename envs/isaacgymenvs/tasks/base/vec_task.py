...
from envs.isaacgymenvs.task_semantics import apply_no_dense_reward_semantics
from core.common.tf_numpy import quaternion_distance
...
class VecTask(Env):
    ...
    def allocate_buffers(self):
        ...
        self.max_linvel_buf = torch.zeros(
            self.num_envs, device=self.device, dtype=torch.float)
        self.stable_max_linvel_buf = torch.zeros(
            self.num_envs, device=self.device, dtype=torch.float)
        self.stable_stage_step_count_buf = torch.zeros(
            self.num_envs, device=self.device, dtype=torch.long)
        ...
        self.env2index = -torch.ones(
            self.num_envs, device=self.device, dtype=torch.long)
        self.source_config_index_buf = -torch.ones(
            self.num_envs, device=self.device, dtype=torch.long)
        # Realworld Buffer
    ...
    def post_physics_step(self):
        ...
        if len(env_ids) > 0 and not self.cfg.get("real_robot", False):
            ...
            if self.cfg.get("paired_stage_eval", False):
                completed = self.paired_record_initialized[env_ids]
                self.paired_terminal_recorded[env_ids] |= completed
    ...
    def domain_randomization_init(self):
        ...
        self.dr_settings["noise"].update({
            ...
            "gripper_delay": [0., 0.1], # half-range around the nominal delay
        })
        ...
        self.dr_settings["controller"].update({
            ...
            "gripper_delay": [0.2, 0.2], # nominal delay; noise gives U(0.1, 0.3)
        })
    ...
    def obs_act_rew_init(self):
        ...
        # Add custom rewards
        task_dense_reward_keys = tuple(self.reward_settings)
    ...
    def configs_related(self):
        ...
        if self.cfg.get("fixed_configs", False) or self.cfg.get("knn_configs_eval", False):
            if self.cfg.get("global_configs", False):
                json_file = os.path.join(self.cfg["eval_res_dir"], self.cfg["task_name"], self.cfg["global_ckpt"]+f"_EVAL_{self.cfg['global_index_episode']}", "trajectories", "init_configs.json")
            elif self.cfg.get("par_configs", False): # update previous configs
                json_file = os.path.join(self.cfg["eval_res_dir"], self.cfg["task_name"], self.cfg["par_checkpoint"]+f"_EVAL_{self.cfg['par_index_episode']}", "trajectories", "init_configs.json")
            else:
                json_file = os.path.join(self.cfg["eval_res_dir"], self.cfg["task_name"], self.cfg["checkpoint"]+f"_EVAL_{self.cfg['index_episode']}", "trajectories", "init_configs.json")
            ...
            self._knn_reference_features = None
            fixed_repeats = self.cfg.get("fixed_config_repeats_eval", None)
            if fixed_repeats is not None:
                requested_configs = self.num_envs // fixed_repeats
                if self.num_envs % fixed_repeats != 0:
                    raise ValueError(
                        "num_envs must be divisible by fixed_config_repeats_eval"
                    )
                if requested_configs > self.num_configs:
                    raise ValueError(
                        f"Requested {requested_configs} fixed configurations but "
                        f"the bank contains only {self.num_configs}"
                    )
    ...
    def get_config_idx(self, env_ids):
        ...
        if self.cfg.get("fixed_configs", False):
            ...
            elif self.cfg.get("fixed_config_repeats_eval", None) is not None:
                config_idx = repeated_fixed_config_indices(
                    env_ids,
                    num_envs=self.num_envs,
                    repeats=int(self.cfg["fixed_config_repeats_eval"]),
                    bank_size=self.num_configs,
                )
            else:
                config_idx = torch.randint(0, self.num_configs, (len(env_ids),), device=self.device)

            self.source_config_index_buf[env_ids] = config_idx

        return config_idx


    def _apply_knn_reference_targets(self, env_ids, task_config, k=5):
        """Set fresh configurations' temporal targets from a frozen reference bank.

        This path is evaluation-only. It leaves the newly sampled task state
        untouched and averages ``time_used`` (T_min) and ``max_linvel``
        (P_max) over the five closest entries in the parent configuration bank.
        """
        if not self.cfg.get("knn_configs_eval", False) or len(env_ids) == 0:
            return
        if self.training:
            raise RuntimeError("knn_configs_eval must not be used for training")
        if not hasattr(self, "env_configs"):
            raise RuntimeError("5-NN evaluation reference bank was not loaded")

        if self._knn_reference_features is None:
            self._knn_reference_features = self._compute_init_config_features(
                self.env_configs
            ).float()
        query_features = self._compute_init_config_features(task_config).float()
        neighbor_count = min(int(k), int(self._knn_reference_features.shape[0]))
        if neighbor_count <= 0:
            raise RuntimeError("5-NN evaluation reference bank is empty")

        distances = torch.cdist(query_features, self._knn_reference_features)
        ...
        init_t2e = self.env_configs["time_used"][neighbor_indices].mean(dim=1)
        init_limvel = self.env_configs["max_linvel"][neighbor_indices].mean(dim=1)
        # Match the fixed-configuration path in ``_reset_timeaware_states``:
        # a requested real-time goal defines the average temporal-control
        # ratio from the freshly estimated T_min.  The task reset samples the
        # fresh state after the generic reset has run, so this cannot be done
        # earlier when ``config_index`` is intentionally None.
        if self.goal_time is not None:
            self.time_ratio_buf[env_ids] = init_t2e / self.goal_time
    ...
    def argument_related(self):
        ...
        self.stable_max_linvel_accm = []
        self.sum_linvel_accm = []
        self.paired_record_initialized = torch.zeros(
            self.num_envs, device=self.device, dtype=torch.bool
        )
        self.paired_terminal_recorded = torch.zeros(
            self.num_envs, device=self.device, dtype=torch.bool
        )
        self.extras["franka"] = {
    ...
    def stage_wise_ctrl_related(self):
        ...
        if self.use_staged_ctrl:
            ...
            self.real_time_milestone = torch.zeros(
                (self.num_envs, len(budget_portion)), device=self.device
            )
    ...
    def _update_stage_wise_time_ratio(self):
        if self.use_staged_ctrl:
            ...
            to_next_stage_env_ids = torch.where(
                self.time_cur_buf
                >= self.real_time_milestone[all_env_ids, self.cur_stage]
            )[0]
    ...
    def _update_scene_instability(self, real=False):
        ...
        stable_mask = torch.ones(self.num_envs, dtype=torch.bool, device=self.device)
        if self.use_staged_ctrl:
            stable_mask = self.speed_describe_tensor[
                self.all_env_ids, self.cur_stage
            ] == 0
        self.stable_max_linvel_buf = update_masked_peak(
            self.stable_max_linvel_buf,
            self.sce_linvel_buf,
            stable_mask,
        )
        self.stable_stage_step_count_buf += stable_mask.to(torch.long)
    ...
    def _update_timeaware_states(self, real=False):
        ...
        # Attribute the just-observed transition to the ratio and stage that
        # generated its action.  Only then advance the controller schedule for
        # the next observation/action.
    ...
    def _update_common_info(self, reward_records):
        self.extras.update({
            ...
            "eps_stable_max_scevel": self.stable_max_linvel_buf.flatten(),
            "eps_stable_stage_steps": self.stable_stage_step_count_buf.flatten(),
        ...
        if self.cfg.get("paired_stage_eval", False):
            self.extras["paired_config_row"] = self.env2index.clone()
            self.extras["source_config_index"] = (
                self.source_config_index_buf.clone()
            )
            self.extras["eps_time_reference"] = self.time2end_init.clone()
            self.extras["stage_time_ratios"] = self.stage_time_ratio_buf.clone()
            self.extras["stage_end_times"] = self.real_time_milestone.clone()
    ...
    def _reset_timeaware_bufs(self, env_ids):
        ...
        self.stable_max_linvel_buf[env_ids] = 0.
        self.stable_stage_step_count_buf[env_ids] = 0
        ...
        stage_reset_ids = env_ids
        if self.cfg.get("paired_stage_eval", False):
            # Strict paired evaluation consumes only the first episode from
            # each environment. Isaac Gym still resets earlier-finished
            # environments while the remaining environments finish. Those
            # recycled trajectories are excluded by the evaluator, so do not
            # recompute (or range-check) an irrelevant second schedule from a
            # terminal-stage ratio.
            stage_reset_ids = env_ids[
                ~self.paired_terminal_recorded[env_ids]
            ]
        self.recompute_staged_time_ratio(stage_reset_ids)
    ...
    def apply_timeaware_rewards(self, rewards, reset_buf, success_buf):
        ...
        reward_records = {}
        # Timeaware rewards
        eps_time_diff = torch.zeros_like(real_time_to_end)
        eps_time_reward = torch.zeros_like(real_time_to_end)
        success_flag = success_buf == 1
        failed_flag = (reset_buf == 1) & (success_buf == 0)

        eps_time_diff[success_flag] = real_time_to_end[success_flag]
        abs_eps_time_diff = torch.abs(eps_time_diff)
        if self.ratio_range is None:
            # Both explicit formulations replace the sparse success reward.
            # The legacy additive formulation remains available for checkpoint
            # reproduction when neither option is enabled.
            if cfg.get("paper_time_optimal_reward", False):
                if reward_settings["r_epstime_scale"] > 0:
                    paper_reward = paper_time_optimal_terminal_reward(
                        eps_time_diff,
                        reward_settings["r_epstime_scale"],
                    )
                    rewards = torch.where(success_flag, paper_reward, rewards)
            elif cfg.get("normalized_time_optimal_reward", False):
                if reward_settings["r_epstime_scale"] > 0:
                    normalized_reward = normalized_time_optimal_terminal_reward(
                        eps_time_diff,
                        self.max_eps_time,
                        reward_settings["r_epstime_scale"],
                    )
                    rewards = torch.where(success_flag, normalized_reward, rewards)
            else:
                # Maximize time2end to become faster.
                eps_time_reward = eps_time_diff
        else:
        ...
        reward_records["eps_time_p"] = abs_eps_time_diff
        if (
            reward_settings["r_epstime_scale"] > 0
            and not (
                self.ratio_range is None
                and (
                    cfg.get("paper_time_optimal_reward", False)
                    or cfg.get("normalized_time_optimal_reward", False)
                )
            )
        ):
    ...
    def record_init_configs(self, env_ids):
        if self.cfg.get("paired_stage_eval", False):
            env_ids = env_ids[~self.paired_record_initialized[env_ids]]
            if len(env_ids) == 0:
                return
        if "init_configs" not in self.extras:
            ...
            init_configs["stable_max_linvel"] = []
            init_configs["sum_linvel"] = []
            if self.cfg.get("paired_stage_eval", False):
                init_configs["paired_env_id"] = []
        ...
        init_configs["stable_max_linvel"].extend([0] * len(env_ids))
        init_configs["sum_linvel"].extend([0] * len(env_ids))
        if self.cfg.get("paired_stage_eval", False):
            init_configs["paired_env_id"].extend(env_ids.cpu().tolist())
        self.time_used_accm.extend([0] * len(env_ids))
        self.max_linvel_accm.extend([0] * len(env_ids))
        self.stable_max_linvel_accm.extend([0] * len(env_ids))
        ...
        if self.cfg.get("paired_stage_eval", False):
            self.paired_record_initialized[env_ids] = True
    ...
    def record_post_config(self, success_ids):
        if "init_configs" not in self.extras: # Maybe warmup stage
            return
        if self.cfg.get("paired_stage_eval", False):
            success_ids = success_ids[~self.paired_terminal_recorded[success_ids]]
        ...
        batch_stable_max_linvel = self.stable_max_linvel_buf[success_ids].cpu().tolist()
        ...
        for i in range(len(success_ids)):
            ...
            self.stable_max_linvel_accm[buffer_index[i]] += batch_stable_max_linvel[i]
            ...
            if self.success_counts[success_ids[i]] >= self.save_threshold:
                ...
                init_configs["stable_max_linvel"][buffer_index[i]] = self.stable_max_linvel_accm[buffer_index[i]] / self.success_counts[success_id].item()
    ...
    def recompute_staged_time_ratio(self, reset_ids=[]):
        if self.use_staged_ctrl and (len(reset_ids) > 0):
            ...
            invalid_avg_time_ratio = (
                (avg_time_ratio < self.ratio_range[0]).any()
                or (avg_time_ratio > self.ratio_range[1]).any()
            )
            if invalid_avg_time_ratio:
            ...
            self.stage_time_ratio_buf[reset_ids] = compute_staged_time_ratios(
                avg_time_ratio,
                self.budget_portion,
                self.speed_describe,
                self.ratio_range,
                use_average_speed=self.cfg.get("use_avg_speed", False),
            )
            stage_ends = compute_stage_end_times(
                self.real_time2end_init[reset_ids],
                self.budget_portion,
            )
            self.real_time_milestone[reset_ids] = stage_ends
            self.cur_stage[reset_ids] = 0
            self.update_time_ratio_buf(
                self.stage_time_ratio_buf[reset_ids, 0], env_ids=reset_ids
            )
            self.update_linvel_gt(reset_ids)
    ...
    def apply_disturbances(self):
        states = self.states
        # Get envs need to apply disturbance
