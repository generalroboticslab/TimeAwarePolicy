...
class RealSenseCamera:
    def __init__(self, img_w=848, img_h=480, fps=60, serial_number=None, enable_depth=True) -> None:
        # Configure depth and color streams
        # 640x480 both 60 fps; 1280x720 depth both 30 fps
        ...
        # Start streaming
        self.start_stream()
        self.aligner = rs.align(rs.stream.color) if enable_depth else None # Align depth to color optical frame
    ...
    def refresh_intrinsics(self, color_frame, depth_frame=None):
        self.refresh_color_intrinsics(color_frame)
        if depth_frame is not None:
            self.refresh_depth_intrinsics(depth_frame)


    def refresh_color_intrinsics(self, color_frame):
        ...
        self.color_intrin_mat = np.array([[self.color_intrinsics.fx, 0, self.color_intrinsics.ppx],
                                          ...
                                          [0, 0, 1]])
        self.distortion_coef = np.array(self.color_intrinsics.coeffs) # All 0 seems

    
    def refresh_depth_intrinsics(self, depth_frame):
